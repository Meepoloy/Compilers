 ...
 * -------------------------------------------------------------
 */
weaver_void bErrorPrint(weaver_string fmt, ...);
weaver_void displayBuffer(Buffer* ptr_Buffer);
weaver_long getFileSize(weaver_string fname);
weaver_intg isNumber(const weaver_string ns);
weaver_void startReader(weaver_string, weaver_string, weaver_char, weaver_intg, weaver_intg);
...
weaver_intg mainReader(weaver_intg argc, weaver_string* argv) {

	/* Create source input buffer */
	weaver_string program = argv[0];
	weaver_string input = argv[2];
	weaver_char mode = MODE_FIXED;
	weaver_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
weaver_void startReader(weaver_string program, weaver_string input, weaver_char mode, weaver_intg size, weaver_intg increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	weaver_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	weaver_char symbol;			/* symbol read from input file */
...
weaver_void bErrorPrint(weaver_string fmt, ...) {
	...
	(weaver_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
